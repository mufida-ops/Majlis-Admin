...
import { getMessageImageUrl } from '@/lib/repositories/threads';

// Module-level cache so re-rendering the same message doesn't re-request the
...
export function useMessageImage(storagePath: string | null | undefined) {
  ...
  useEffect(() => {
    ...
    const existing =
      ...
      getMessageImageUrl(storagePath).then(u => {
